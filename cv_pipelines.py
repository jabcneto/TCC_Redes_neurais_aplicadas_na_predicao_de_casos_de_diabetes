...
def run_nested_cross_validation(x_train, y_train, x_val, y_val, n_folds, max_trials):
...
def run_cross_validation_with_pretrained(x_train, y_train, x_val, y_val, x_test, y_test, n_folds, load_model):
    ...
    cv_results = cross_validation.cross_validate_mlp(
        ...
        n_folds=n_folds,
        epochs=100,
        batch_size=best_hps_dict.get('batch_size', 64)
    )
...
def run_cross_validation_after_tuning(best_hps, tuned_metrics, x_train, y_train, x_val, y_val, n_folds):
