...
def cross_validate_mlp(model_builder, x_train, y_train, n_folds=5, epochs=100, batch_size=64, patience=15):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(x_train, y_train), 1):
        ...
        LOGGER.info(f"  F1-Score: {fold_metrics['f1_score']:.4f}")
        LOGGER.info(f"  ROC-AUC: {fold_metrics['roc_auc']:.4f}")
        LOGGER.info(f"  Balanced Accuracy: {fold_metrics['balanced_accuracy']:.4f}")
        LOGGER.info(f"  Épocas treinadas: {fold_metrics['epochs_trained']}")
    ...
    metrics_to_show = ['accuracy', 'precision', 'recall', 'f1_score', 'roc_auc', 'balanced_accuracy']
    ...
    best_fold_idx = df_results['f1_score'].idxmax()
    best_model = fold_models[best_fold_idx]

    LOGGER.info(f"\n🏆 Melhor Fold: {best_fold_idx + 1}")
    LOGGER.info(f"  F1-Score: {df_results.loc[best_fold_idx, 'f1_score']:.4f}")
    LOGGER.info(f"  Precision: {df_results.loc[best_fold_idx, 'precision']:.4f}")
    LOGGER.info(f"  Recall: {df_results.loc[best_fold_idx, 'recall']:.4f}")
...
def compare_cv_with_holdout(cv_results, holdout_metrics):
    LOGGER.info("\n" + "=" * 80)
    LOGGER.info("COMPARAÇÃO: VALIDAÇÃO CRUZADA vs HOLDOUT")
    LOGGER.info("=" * 80)
    ...
    comparison_path = os.path.join(RESULTS_DIR, 'cross_validation', 'cv_vs_holdout.csv')
...
def nested_cross_validation(model_builder_func, hyperparameter_space, x_train, y_train,
    ...
    for outer_fold_idx, (train_idx, test_idx) in enumerate(outer_skf.split(x_train, y_train), 1):
        ...
        early_stop = EarlyStopping(monitor='val_precision', patience=10, mode='max', restore_best_weights=True)
        ...
        LOGGER.info(f"  ROC-AUC: {outer_fold_metrics['roc_auc']:.4f}")
