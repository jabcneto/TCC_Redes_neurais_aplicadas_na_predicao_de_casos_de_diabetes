...
A técnica SMOTE também foi aplicada para mitigar o desbalanceamento de classes no conjunto de treino. Ela cria amostras sintéticas da classe minoritária, após tratamento de outliers, codificação one-hot e padronização por Z-score, evitando vazamento de informação, os conjuntos de validação e teste não foram reajustados. Esta estratégia aumentou a representatividade da classe minoritária como podemos ver na figura \ref{fig:smite}, e reduziu o viés de previsão e melhorou métricas como recall, sem comprometer a avaliação do modelo.
...
Do conjunto de dados original, 20\% foram reservados para validação. O restante foi então dividido de forma estratificada, com 80\% para treino. A utilização de uma semente aleatória fixa garantiu a reprodutibilidade dos resultados. Essa metodologia permite uma avaliação imparcial com dados nunca antes vistos e a calibração precisa dos modelos através do conjunto de validação. 
...
A matriz é composta por quatro elementos principais que podemos visualisar na figura \ref{fig:cf}
...
A fórmula para calcular a Acurácia pode ser vista na formula \ref{eq:Accuracy}.
...
A fórmula para calcular a Precisão pode ser vista na formula \ref{eq:precision}.
\begin{equation}
    Precision =\frac{\text{correctly classified actual positives}}{\text{everything classified as positive}} = \frac{VP}{VP+FP} 
    \label{eq:precision}
\end{equation}
...
A fórmula para calcular o Recall pode ser vista na formula \ref{eq:recall}.
\begin{equation}
...
A fórmula para calcular o F1 Score pode ser vista na formula \ref{eq:F1_Score}.
...
\subsection{Os Modelos Escolhidos}
...
Para uma análise de desempenho comparativa e aprofundada, indo além da métrica de acurácia, foram geradas Matrizes de Confusão para todos os modelos avaliados: MLP e CNN. Estas matrizes, apresentadas nas figuras \ref{fig:cm} e \ref{fig:cm_cnn}, são cruciais no contexto do diagnóstico de diabetes, pois permitem avaliar os tipos de erros específicos que cada arquitetura comete.
...
\begin{figure}[H]
    ...
    \label{fig:cm_cnn}
    \includegraphics[width=0.8\textwidth]{figuras/cm_cnn.png} 
    \par 
     \small Fonte: Elaborado pelo autor
\end{figure}
...
Observa-se que o modelo MLP superou ligeiramente o modelo que utiliza a arquitetura CNN, embora a diferença não seja muito significativa, mas ao comparar o desempenho temporal das duas arquiteturas ao longo de 20 repetições de 100 épocas, notou-se uma diferença clara de custo computacional. A MLP manteve-se consistentemente mais rápida, finalizando o treinamento em uma média de uma hora e 28 minutos. Em contrapartida, a CNN levou em média 1 hora e 54 minutos, o que representa um aumento de tempo esperado devido às operações matemáticas adicionais que essa rede realiza. A utilização da média de 20 trials assegura que esse resultado não foi um caso isolado, mas sim um comportamento padrão dos modelos.
