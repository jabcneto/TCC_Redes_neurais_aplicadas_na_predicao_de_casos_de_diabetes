...
Na arquitetura básica de um neurônio artificial demonstrada na figura 2, podemos observar que ele opera sobre sinais de entrada, os quais são valores numéricos ponderados pelos pesos sinápticos, podendo ser oriundos de outros neurônios ou de uma fonte inicial de dados \cite{Busnardo2024}.
...
\section{IDE: PyCharm}
O PyCharm é uma das IDEs (Integrated Development Environment) mais renomadas para a programação em Python, desenvolvida pela JetBrains. Ela se destaca pela combinação de uma interface intuitiva que observamos na figura 6 e uma ampla gama de recursos avançados, tornando-se uma ferramenta essencial para desenvolvedores de todos os níveis\cite{Jetbrains202x, Rocha2021, Pylinton2021}.
